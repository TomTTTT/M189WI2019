...
###Method
Palindromes are symmetrical orderings of DNA such that they read exactly the same as their reverse complementary sequences. This analysis aims to see if clusters of palindromes are significant enough to provide more information about replication origins of CMV DNA. The analysis was conducted using a data set obtained by M.S. Chee concerning the location of palindromes in a DNA sequence of CMV (cytomegalovirus). Methods used in this analysis include Maximum Likelihood Estimation, Poisson Process, Monte Carlo simulation, and Chi-Square Goodness of Fit testing.  

###Results
Through uniform scatters, we found that it is hard to tell if there are significant clusters in the same way that there are in the original dataset. We also found that the distributions of spacings between pairs, triplets, decuples appeared to follow an exponential, gamma, and normal distribution respectively. However, we found that our data is not uniformly distributed and that it does not follow a Poisson Process. 

###Conclusion
Our analysis shows the presence of palindome clusters around the 93,000th and 190,000th sequences in CMV DNA sequence. 
...
#Counts
We proceed with a Chi Square test on our original data to determine if our data does in fact follow poisson distribution. We split our original DNA sequence in 3 different ways:  
  
First we split into 76 intervals of size 3000 and used an estimated $\hat{\lambda}$ of $\frac{296}{76}= 3.87$.
  
  
Second we split into 50 intervals of size 4500 and used an estimated $\hat{\lambda}$ of $\frac{296}{50}=5.78$.
  
 
Third we split into 38 intervals of size 6000 and used an estimated $\hat{\lambda}$ of $\frac{296}{38}=7.74$.
  
  
We proceed to test
$$H_0 : Our\; data\; follows\; a \;poisson \;distribution \;with\; estimated\; \hat{\lambda}$$
$$H_1: Our \;data\; does\; not\; follow\; a \;poisson \;distribution \;with\; estimated\; \hat{\lambda}$$ 
From our original data we looked at the counts of palindromes in each of the separate intervals and found the frequencies in which those counts occurred, our data is summarized in the following tables.
...
Note: In these tables we combined several of the bins so that all of the expected frequencies are greater than 5 which is a condition for the Chi-Square test. Furthermore, the intervals for our data are non overlapping and independent therefore the conditions for the Chi-Square test are met. Moving forward, we tested the null hypothesis on each of the different bin sizes using a Chi-Square test with degrees of freedom =(#of intervals)-1-1. Here we subtracted an additional degree of freedom due to the fact the estimate lambda given our original data.
The results of the Chi-Square tests is summarized in the following table.
...
chisq_df <- data.frame(binwidth = c(3000, 4500, 6000),
                            lamda= c(c3m,c4m,c6m),
                            chisq_value=c(c3m_chi_square_val,c4m_chi_square_val,c6m_chi_square_val),
                            dof=c(nrow(cleaned_poisson_3) - 2,nrow(cleaned_poisson_4) - 2,nrow(cleaned_poisson_6) - 2),
                            value = c(0.0005707537, 0.342621, 0.1102576))

knitr::kable(chisq_df,
             row.names = FALSE,
             col.names = c("Interval Size","$\\hat{\\lambda}$","$\\chi^2$", "degrees of freedom", "p-value"), booktabs = TRUE,
             caption = "$\\chi^2$ Values With Different Interval Sizes", escape = FALSE)
```

Notice that with interval sizes of 4500 and 6000 we fail to reject the null hypothesis so we cannot conclude that the data did not come from a poisson distribution with those interval sizes and $\hat{\lambda}$. Yet, when we look at the interval width of 3000 we get a very small p-value and do in fact reject the null hypothesis with an $\alpha$= .01 significance level. Here we note that changing the interval size changes the behavior of the p-value and is something we should keep in mind as we look to identify clusters of palindromes.

#The Biggest Cluster
```{r, echo=F,message=FALSE, warning=FALSE, results=FALSE}
################
## simulation (part 4) ##
################
library(OneR)
...
We calculate the p-value using the equation 1 - P(maximum count over m intervals $\ge$ k). We find that the p-value is $3.315838 \times 10^{-4}$, rejecting our null hypothesis. So, we do in fact reason that there is an unreasonably large cluster to indicate a potential origin of replication in the interval where the max number of palindromes are, interval 24. 

To check that our p-value function is reliable, we checked its power. We sampled 296 data points from uniform distribution and binned the data points into 57 intervals just like our test. Then we used 1 - P(maximum count over m intervals $\geq$ k) as our p-value and checked, with 1000 simulations, how many times we would fail to reject the null hypothesis, same as above. We calculated the power by subtracting the number of times we failed to reject divided by how many times we simulated to get $\beta$. Then, we subtract $\beta$ from 1 to get power. 

$$\beta = \frac{number\; of\; type\; 2 \;errors}{number\; of\; simulations}$$
$$Power = 1 - \beta$$

Using the function, we get a power of 0.047. The low power value indicates that fails to reject a lot. Since we are using a uniformly distributed sample and we are testing for whether or not we have an abnormally large cluster as our null hypothesis, we know that it doesn't have an abnormally large cluster and the test should have a low power. So the p-value function is reliable to use.


#Effects of HIV and Hypertension on HCMV IgG

We looked at a data set regarding human cytomegalovirus and its relationship to tuberculosis and cardiovascular disease risk factors in a rural Ugandan cohort. We wanted to see if there was a relationship between HCMV IgG (Immunoglobulin G) and Hypertension, and HCMV IgG and HIV status. We hypothesizes that there exists a relationship between those who have health impairments to their immune system and high blood pressure (hypertension). As you can see in the figure below, we show the quantiles of the following relationships. 
...
box_hiv<-ggplot(df3, aes(x=hiv, y=cmv)) + geom_boxplot(color="black", fill="red", alpha=0.2) +
  ...
  ggtitle("HIV Status") + ylab("HCM IgG")

box_hyper<-ggplot(df3, aes(x=R22_bpgroup, y=cmv)) + geom_boxplot(color="black", fill="red", alpha=0.2) +
  theme(axis.title.x=element_blank(),axis.text.x = element_text(angle = 90, hjust = 1), plot.title = element_text(hjust = 0.5)) +
  ggtitle("Hypertension Status") + ylab("HCM IgG")
...
###Gamma distribution 
The gamma distribution is another distribution that is relevant to the Poisson process. The Gamma distribution has a parent function in the exponential distribution. Since we are working with a Homogeneous Poisson process model, we know that the distance between consecutive hits has an exponential distribution. Similarly, the distance between two hits had a Gamma distribution. A unique property of the Gamma distribution is that it requires two parameters denoted as $\alpha$ and $\lambda$. The Gamma distribution utilizes the Gamma function which is defined by 

$$\Gamma(k) = \int_0^{\infty}x^{k-1}e^{-x}dx, \;\;k \in (0,\infty)$$

     
The integral converging for any k > 0 means the function is well defined or sometimes referred to as a complete Gamma function. Otherwise, the integral diverges for any $k\le0$. Now if a random variable $X$ has a Gamma distribution ($\alpha$,$\lambda$) if
$$
\begin{cases}
\frac{1}{\Gamma(\alpha)}\lambda^{\alpha}x^{\alpha-1}e^{-x\lambda} & x\ge 0\\
0 & x<0
\end{cases}
$$
...
Our analysis shows that the clusters occur around the 93,000th and 190,000th sequences of DNA base pairs. We found that the largest cluster of palindromes in the particular sequence of CMV DNA was statistically significant, which raises questions about the origin of replication for this sequence of DNA. In being able to find significant large clusters, this would enable researchers to understand where CMV is likely to replicate along a strand of DNA to find ways to combat replication of this virus in human bodies. We also found that HIV status and hypertension status are related to the severity of HCMV IgG, which should raise questions about weakened health and immune system may be affected by the presence of CMV in a human body. 
